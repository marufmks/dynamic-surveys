...
if(!class_exists('Dynamic_Surveys_Frontend')) {  
    class Dynamic_Surveys_Frontend
    {
        ...
        function frontend_scripts()
        {
            ...
            wp_enqueue_style('toastr-css', DYNAMIC_SURVEYS_PLUGIN_URL . '/assets/css/toastr.min.css', array(), DYNAMIC_SURVEYS_PLUGIN_VERSION);
            wp_enqueue_style('dynamic-surveys-frontend-style', DYNAMIC_SURVEYS_PLUGIN_URL . 'assets/css/dynamic-surveys-frontend.css', array(), DYNAMIC_SURVEYS_PLUGIN_VERSION);
    
            wp_enqueue_script('chart-js', DYNAMIC_SURVEYS_PLUGIN_URL . 'assets/js/chart.js', array(), DYNAMIC_SURVEYS_PLUGIN_VERSION, true);
            wp_enqueue_script('toastr-js', DYNAMIC_SURVEYS_PLUGIN_URL . '/assets/js/toastr.min.js', ['jquery'], DYNAMIC_SURVEYS_PLUGIN_VERSION, true);
            wp_enqueue_script(
                'dynamic-surveys-frontend-script',
                DYNAMIC_SURVEYS_PLUGIN_URL . 'assets/js/dynamic-surveys-frontend.js',
                array('jquery', 'chart-js', 'toastr-js', 'wp-i18n', 'wp-escape-html'),
                DYNAMIC_SURVEYS_PLUGIN_VERSION,
                true
            );
        ...
        function dynamic_surveys_shortcode($atts)
        {
            ...
            ob_start();
            include DYNAMIC_SURVEYS_PLUGIN_PATH . 'templates/frontend/dynamic-surveys-frontend-display.php';
            return ob_get_clean();
        }
    }
}
